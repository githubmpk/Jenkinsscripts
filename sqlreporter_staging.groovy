...
job ('sqlreporter-deploy-staging') {
    blockOn('/.*-deploy/')
    scm {
        git {
            remote {
    ...
    triggers {
        upstream('sqlreporter-build-staging', 'SUCCESS')
    }
    steps {
...
job ('sqlreporter-move-staging') {
    blockOn('/.*-deploy/')
    steps {

        triggers {
            upstream('sqlreporter-deploy-staging', 'SUCCESS')
        }
