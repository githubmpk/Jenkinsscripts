job('pathprovider-build-production') {
    scm {
        git {
            remote {
...
job ('pathprovider-deploy-production') {
    blockOn('/.*-deploy/')
    scm {
        git {
            remote {
    ...
    triggers {
        upstream('pathprovider-build-production', 'SUCCESS')
    }
    steps {
        ...
        copyArtifact {
            projectName('pathprovider-build-production')
...
job ('pathprovider-functionalTest-production') {
    blockOn('/.*-deploy/')
    scm {
        git {
            remote {
    ...
    triggers {
        upstream('pathprovider-deploy-production', 'SUCCESS')
    }
    steps {
        gradle {
            gradleName('Gradle Local')
...
job ('pathprovider-move-production') {
    blockOn('/.*-deploy/')

    triggers {
        upstream('pathprovider-functionalTest-production', 'SUCCESS')
    }
...
listView('Path Provider (production)') {
    description('Path Provider')
    jobs {
        regex('pathprovider-.+.-production.*')
    }
    columns {
        status()
        weather()
